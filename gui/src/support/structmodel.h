...
public:
    ...
    QVariant data(QModelIndex const& index,
        ...
        if (index.row() < 0 || index.row() >= m_records.size()) return {};
    ...
    bool setData(QModelIndex const& index,
        ...
        if (index.row() < 0 || index.row() >= m_records.size()) return false;
        if (index.column() < 0 || index.column() >= m_header.size())
            return false;
    ...
    bool insertRows(int                row,
        ...
        if (row > m_records.size()) return false;
...
public:
    ...
    bool setData(QModelIndex const& index,
                 ...
                 int                role = Qt::EditRole) override {
        if (!index.isValid()) return false;
        if (index.row() < 0 || index.row() >= this->m_records.size())
            return false;
        if (index.column() < 0 || index.column() >= this->m_header.size())
            return false;
    ...
    bool insertRows(int                row,
                    ...
                    QModelIndex const& p = QModelIndex()) override {
        if (row < 0 or count <= 0) return false;
        if (row > this->m_records.size()) return false;
