...
bool includes_event(EventTypeContainer const& filter, SolTrace::GUI::Data::RayEventType event) {
    return filter.events.contains(event);
}

bool includes_entity(SolTrace::GUI::Data::RayRecord const& path, SolTrace::GUI::Data::Entity entity) {
...
double extract_total_ray_distance(SolTrace::GUI::Data::RayRecord const&      path,
...
EventTypeContainer::EventTypeContainer(
    std::initializer_list<SolTrace::GUI::Data::RayEventType> l)
    : events(l) { }
...
EventTypeContainer::EventTypeContainer(QStringList l) {
    for (auto const& item : l) {
        ...
        auto maybe_enum = magic_enum::enum_cast<SolTrace::GUI::Data::RayEventType>(str);
...
void RayGeometry::rebuild_geometry() {
    // Kickoff
    qCDebug(rayGeometryLog) << Q_FUNC_INFO << "Start";
    clear();
    ...
    if (!m_database) {
        qCDebug(rayGeometryLog) << Q_FUNC_INFO << "No database";
    ...
    qCDebug(rayGeometryLog) << Q_FUNC_INFO << vertex_count;
    ...
    qCDebug(rayGeometryLog) << Q_FUNC_INFO << "New buffers ready";
    ...
    qCDebug(rayGeometryLog) << Q_FUNC_INFO << "Update";
    update();
}

void RayGeometry::inclusion_list_update() {
    qCDebug(rayGeometryLog) << Q_FUNC_INFO << "List changed";
...
void RayGeometry::selected_ray_change_update_table() {
    ...
    static auto names = []() {
        QHash<SolTrace::GUI::Data::RayEventType, QString> ret;

        for (auto [value, name] :
             magic_enum::enum_entries<SolTrace::GUI::Data::RayEventType>()) {
            auto str =
...
RayGeometry::RayGeometry(QQuick3DObject* parent)
    ...
    m_include_events = EventTypeContainer({
        SolTrace::GUI::Data::RayEventType::ABSORB,
        SolTrace::GUI::Data::RayEventType::REFLECT,
        SolTrace::GUI::Data::RayEventType::TRANSMIT,
    });
...
void RayGeometry::set_results(SolTrace::GUI::Data::SimulationResultPtr data) {
    qCDebug(rayGeometryLog) << Q_FUNC_INFO << "New ray geometry database";
...
void RayGeometry::select_entity_filter(SolTrace::GUI::Data::Entity entity) {
...
/// tolerance
static RayCastRayResult check_distance(SolTrace::GUI::Data::RayRecord const& record,
...
void RayGeometry::pick_ray(QVector3D world_position,
    ...
    RayCastRayResult result = QtConcurrent::blockingMappedReduced(
        start_iter,
        end_iter,
        [=](SolTrace::GUI::Data::RayRecord const& record) {
    ...
    if (result.ray_id < 0) {
        qCDebug(rayGeometryLog) << Q_FUNC_INFO << "No hit";
        return;
    }

    qCDebug(rayGeometryLog) << Q_FUNC_INFO << "Hit" << result.ray_id;
