...
void AnInstanceEditor::an_entity_changed(SolTrace::GUI::Data::Entity e) {
    if (m_entity == e) {
        recompute();
        return;
    }
...
void AnInstanceEditor::set(SolTrace::GUI::Data::Entity ent) {
    m_euler_angles_xyz_valid = false;
    set_entity(ent);
    recompute();
}
...
void AnInstanceEditor::reset(Database* database) {
    ...
    connect(database->global_transform.self(),
            &ComponentAPIBase::changed,
            this,
            [this](SolTrace::GUI::Data::Entity e) {
...
SolTrace::GUI::Data::Entity AnInstanceEditor::material_group() const {
    if (m_host and m_host->valid(m_entity)) {
...
void AnInstanceEditor::set_material_group(SolTrace::GUI::Data::Entity newGroup) {
...
SolTrace::GUI::Data::Entity AnInstanceEditor::geometry_group() const {
    if (m_host and m_host->valid(m_entity)) {
...
void AnInstanceEditor::set_geometry_group(SolTrace::GUI::Data::Entity newGroup) {
...
SolTrace::GUI::Data::Entity AnInstanceEditor::parent() const {
    if (m_host and m_host->valid(m_entity)) {
...
void AnInstanceEditor::set_parent(SolTrace::GUI::Data::Entity newParent) {
...
QVector<SolTrace::GUI::Data::Entity> AnInstanceEditor::tags() const {
    if (m_host and m_host->valid(m_entity)) {
...
void AnInstanceEditor::set_tags(QVector<SolTrace::GUI::Data::Entity> const& newTags) {
    auto current_tags = tags();

    std::unordered_set<SolTrace::GUI::Data::Entity> incoming(newTags.begin(), newTags.end());
    std::unordered_set<SolTrace::GUI::Data::Entity> current(current_tags.begin(),
                                           current_tags.end());
