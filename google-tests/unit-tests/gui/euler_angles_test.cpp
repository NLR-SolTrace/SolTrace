...
TEST(EulerAnglesXYZ, RoundTripsAwayFromSingularity)
{
    ...
    for (auto const& degrees_case : cases)
    {
        ...
        auto quat     = SolTrace::GUI::Data::euler_xyz_to_quat(expected);
        auto actual   = SolTrace::GUI::Data::compatible_euler_xyz_from_quat(quat, expected);

        expect_vec_near(actual, expected);
        EXPECT_TRUE(same_rotation(SolTrace::GUI::Data::euler_xyz_to_quat(actual), quat));
    }
}
...
TEST(EulerAnglesXYZ, PreservesCompatibleValuesPastOneRevolution)
{
    ...
    auto       quat     = SolTrace::GUI::Data::euler_xyz_to_quat(previous);

    auto actual = SolTrace::GUI::Data::compatible_euler_xyz_from_quat(quat, previous);
...
TEST(EulerAnglesXYZ, KeepsXChannelStableAtNinetyDegrees)
{
    ...
    auto refreshed = SolTrace::GUI::Data::compatible_euler_xyz_from_quat(quat, previous);
    expect_vec_near(degrees(refreshed), { 90.0, 0.0, 0.0 });

    glm::dvec3 edited_y = radians({ 90.0, 22.5, 0.0 });
    quat                = SolTrace::GUI::Data::euler_xyz_to_quat(edited_y);
    refreshed           = SolTrace::GUI::Data::compatible_euler_xyz_from_quat(quat, previous);
    expect_vec_near(degrees(refreshed), { 90.0, 22.5, 0.0 });

    glm::dvec3 edited_z = radians({ 90.0, 22.5, -37.0 });
    quat                = SolTrace::GUI::Data::euler_xyz_to_quat(edited_z);
    refreshed           = SolTrace::GUI::Data::compatible_euler_xyz_from_quat(quat, edited_y);
...
TEST(EulerAnglesXYZ, ChoosesEquivalentSolutionNearestPrevious)
{
    ...
    auto       quat      = SolTrace::GUI::Data::euler_xyz_to_quat(canonical);
