...
{
OpticalDistribution
to_optical_distribution(SolTrace::Data::DistributionType distribution)
{
    using SolTrace::Data::DistributionType;

    switch (distribution)
    {
    case DistributionType::NONE: return OpticalDistribution::OPT_NONE;
    case DistributionType::GAUSSIAN: return OpticalDistribution::OPT_GAUSSIAN;
    case DistributionType::PILLBOX: return OpticalDistribution::OPT_PILLBOX;
    case DistributionType::DIFFUSE: return OpticalDistribution::OPT_DIFFUSE;
    default: throw std::invalid_argument("Unimplemented error distribution");
    }
}
} // namespace
...
CspElement::CspElement()
{
    ...
    set_optics_front(
        false, 1.f, 0.f, 1.f, 1.f, 0.f, 0.f, OpticalDistribution::OPT_NONE);
    set_optics_back(
        false, 1.f, 0.f, 1.f, 1.f, 0.f, 0.f, OpticalDistribution::OPT_NONE);
}
...
void CspElement::set_optics_front(const bool                use_refraction,
...
{
    this->set_optics(true,
                     use_refraction,
                     reflectivity,
                     transmissivity,
                     refractive_index_incident,
                     refractive_index_transmitted,
                     slope_error,
                     specularity_error,
                     od);
}
void CspElement::set_optics_back(const bool                use_refraction,
                                 const float               reflectivity,
                                 const float               transmissivity,
                                 const float               refractive_index_incident,
                                 const float               refractive_index_transmitted,
    ...
    const std::shared_ptr<const SolTrace::Data::OpticalPropertySet>& optics)
{
    ...
    const bool use_refraction = optics->get_interaction_type() ==
                                SolTrace::Data::InteractionType::REFRACTION;

    double refract_index_front, refract_index_back;
    optics->get_refraction_indices(refract_index_front, refract_index_back);
                                        ...
                                        const Vec3d& upper_local)
{
    ...
    std::vector<Vec3d> corners = { g0, g1, g2, g3, g4, g5, g6, g7 };
...
GeometryDataST CspElement::toDeviceGeometryData() const
{
    ...
    if (aperture_type == ApertureType::RECTANGLE)
    {
        ...
        auto   rect_aperture =
            std::dynamic_pointer_cast<ApertureRectangle>(m_aperture);
        double x_coord = rect_aperture->get_x_coord();
        double y_coord = rect_aperture->get_y_coord();

        Matrix33d rotation_matrix =
            get_rotation_matrix(); // L2G rotation matrix
        ...
        {
            Vec3d local_center(
                x_coord + 0.5 * width, y_coord + 0.5 * height, 0);
            Vec3d global_center = rotation_matrix * local_center + m_origin;
            GeometryDataST::Rectangle_Flat heliostat(
                OptixCSP::toFloat3(global_center),
                OptixCSP::toFloat3(v1),
                OptixCSP::toFloat3(v2),
                (float)width,
                (float)height);
        ...
        if (surface_type == SurfaceType::PARABOLIC)
        {
            ...
            // float3 anchor = OptixCSP::toFloat3(m_origin - v1 * 0.5 - v2 *
            // 0.5);
            Vec3d global_anchor = rotation_matrix * local_anchor + m_origin;

            GeometryDataST::Rectangle_Parabolic heliostat(
                OptixCSP::toFloat3(edge_x),
        ...
        if (surface_type == SurfaceType::CYLINDER)
        {
            ...
            GeometryDataST::Cylinder_Y heliostat(
                center, radius, half_height, base_x, base_z);
        ...
        if (surface_type == SurfaceType::SPHERICAL)
        {
            ...
            GeometryDataST::Rectangle_Spherical rs(o,
                                                   vx,
                                                   vy,
                                                   R,
                                                   (float)width,
    ...
    if (aperture_type == ApertureType::TRIANGLE)
    {
        ...
        if (surface_type == SurfaceType::FLAT)
        {
            // given the origin and rotation, compute global coordinates of the
            // triangle vertices
            Matrix33d rotation_matrix =
                get_rotation_matrix(); // L2G rotation matrix
            ...
            GeometryDataST::Triangle_Flat heliostat(
                OptixCSP::toFloat3(v1_global),
        ...
        if (surface_type == SurfaceType::PARABOLIC)
        {
            ...
            // Vec3d x and y components directly (since rotation is
            // orthonormal).
            ...
            GeometryDataST::Triangle_Parabolic trip(
                o, vx, vy, cx, cy, lv0, lv1, lv2);
        ...
        if (surface_type == SurfaceType::SPHERICAL)
        {
            ...
            GeometryDataST::Triangle_Spherical tris(
                o, vx, vy, R, lv0, lv1, lv2);
    ...
    {
        ApertureQuadrilateral quad =
            static_cast<ApertureQuadrilateral&>(*m_aperture);
        ...
        if (surface_type == SurfaceType::FLAT)
        {
            ...
            GeometryDataST::Quadrilateral_Flat heliostat(
                OptixCSP::toFloat3(p1_global),
        ...
        if (surface_type == SurfaceType::PARABOLIC)
        {
            ...
            GeometryDataST::Quadrilateral_Parabolic heliostat(
                ...
                make_float2(p2[0], p2[1]),
                make_float2(p3[0], p3[1]),
                make_float2(p4[0], p4[1]));
    ...
    if (aperture_type == ApertureType::ANNULUS)
    {
        ...
        if (surface_type == SurfaceType::FLAT)
        {
            GeometryDataST::Annulus_Flat anf(
                o, n, vx, vy, radius_in, radius_out, arc);
        ...
        if (surface_type == SurfaceType::PARABOLIC)
        {
            float cx = (float)(m_surface->get_curvature_1());
            float cy = (float)(m_surface->get_curvature_2());
            GeometryDataST::Annulus_Parabolic anp(
                o, vx, vy, cx, cy, radius_in, radius_out, arc);
        ...
        {
            float R = 1.0f / (float)(m_surface->get_curvature_1());
            GeometryDataST::Annulus_Spherical as(
                o, vx, vy, R, radius_in, radius_out, arc);
...
//     if (aperture_type == ApertureType::RECTANGLE && surface_type !=
...
//         m_lower_box_bound[0] = fmin(fmin(corner1_global[0],
//         corner2_global[0]), fmin(corner3_global[0], corner4_global[0]));
//         m_lower_box_bound[1] = fmin(fmin(corner1_global[1],
//         corner2_global[1]), fmin(corner3_global[1], corner4_global[1]));
//         m_lower_box_bound[2] = fmin(fmin(corner1_global[2],
//         corner2_global[2]), fmin(corner3_global[2], corner4_global[2])) -
//         epsilon;

//         m_upper_box_bound[0] = fmax(fmax(corner1_global[0],
//         corner2_global[0]), fmax(corner3_global[0], corner4_global[0]));
//         m_upper_box_bound[1] = fmax(fmax(corner1_global[1],
//         corner2_global[1]), fmax(corner3_global[1], corner4_global[1]));
//         m_upper_box_bound[2] = fmax(fmax(corner1_global[2],
//         corner2_global[2]), fmax(corner3_global[2], corner4_global[2])) +
//         epsilon;
//     }

//     // slightly different for the cylinder, we want to know the radius and
//     half height if (surface_type == SurfaceType::CYLINDER)
//     {
...
//         Matrix33d rotation_matrix = get_rotation_matrix(); // L2G rotation
//         matrix
...
//         // go through the corners and find the min and max x, y, z
//         std::vector<Vec3d> corners = {corner1_global, corner2_global,
//         corner3_global, corner4_global,
//                                       corner5_global, corner6_global,
//                                       corner7_global, corner8_global};
...
//     if (aperture_type == ApertureType::TRIANGLE)
//     {
//         // get the three vertices of the triangle aperture in local
//         coordinates
//         // first cast to ApertureTriangle type
//         ApertureTriangle tri = static_cast<ApertureTriangle &>(*m_aperture);
...
//         Vec3d v3_global = rotation_matrix * v3 + m_origin;
//         // now update the bounding box, need to find the min and max x, y, z
//         m_lower_box_bound[0] = fmin(fmin(v1_global[0], v2_global[0]),
//         v3_global[0]); m_lower_box_bound[1] = fmin(fmin(v1_global[1],
//         v2_global[1]), v3_global[1]); m_lower_box_bound[2] =
//         fmin(fmin(v1_global[2], v2_global[2]), v3_global[2]);
//         m_upper_box_bound[0] = fmax(fmax(v1_global[0], v2_global[0]),
//         v3_global[0]); m_upper_box_bound[1] = fmax(fmax(v1_global[1],
//         v2_global[1]), v3_global[1]); m_upper_box_bound[2] =
//         fmax(fmax(v1_global[2], v2_global[2]), v3_global[2]);
//     }
// }
...
void CspElement::set_optics(const bool                is_front,
